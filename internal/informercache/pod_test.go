...
func TestStripPod(t *testing.T) {
	...
	if !reflect.DeepEqual(pod.Spec, v1.PodSpec{}) || !reflect.DeepEqual(pod.Status, v1.PodStatus{}) || pod.Labels != nil || !IsObjectStripped(pod) {
...
func TestAtomicInitialListWithStrippedPod(t *testing.T) {
	...
	for _, name := range []string{"running-1", "excluded", "running-2"} {
		...
		if IsObjectStripped(obj.(*v1.Pod)) != (name == "excluded") {
