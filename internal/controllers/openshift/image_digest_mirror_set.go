...
func (r *ImageDigestMirrorSetController) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	// Add a small delay before processing the event to wait for the cache sync since it lags behind by definition.
	time.Sleep(200 * time.Millisecond)

	var set v1.ImageDigestMirrorSet
	...
	for _, mirror := range set.Spec.ImageDigestMirrors {
...
func (r *ImageDigestMirrorSetController) SetupWithManager(mgr ctrl.Manager, opts controller.Options) error {
	return ctrl.NewControllerManagedBy(mgr).
		Named("AikidoSecurityWatcher_" + ImageDigestMirrorSetGVK.String() + "_" + uuid.NewString()).
		For(&v1.ImageDigestMirrorSet{}).
		WithOptions(opts).
		Complete(r)
}
